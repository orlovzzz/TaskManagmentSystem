...
function Account(props) {
    ...
    const handleDetailsButton = async (id) => {
        try {
            const response = await axios.get(props.url + '/accountApi/accounts/' + id, {
                headers: { 'Authorization': Cookies.get('token') }
            })
            setAccountDetails(response.data)

            const response1 = await axios.get(props.url + '/taskApi/task/account/' + id, {
                headers: { 'Authorization': Cookies.get('token') }
            })
            setAccountTaskDetails(response1.data);
            console.log(response1);

            const response2 = await axios.get(props.url + "/taskApi/task/executor/" + id, {
                headers: { 'Authorization': Cookies.get('token') }
            })
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(props.url + "/accountApi/accounts", {
                headers: { 'Authorization': Cookies.get('token') }
                });
